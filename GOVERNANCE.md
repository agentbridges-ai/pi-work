...
评审计数按 PR 作者动态执行：`@Misakago` 作者需要至少 1 个针对最新提交的有效 Core 审批；其他 Core 作者需要至少 2 个针对最新提交的有效审批。高风险路径在此基础上仍要求 Leader 作为作者或最新提交批准者参与。Ruleset 负责 Core CODEOWNER 的基础门禁，`governance-review` 只读取 PR 元数据与评审，不读取组织成员清单。
