...
- 非 Leader Core 作者：2 个不同、非作者且在 allowlist 中的当前 head 审批；
- 社区作者：1 个 allowlist 中的当前 head 审批；
- 低风险 Dependabot：1 个当前 head 的 `Misakago` 审批；scope 不合规则回退普通/高风险规则。
...
current-head 约束，从而允许 Leader self-or-exempt。`governance-review` 缺失、allowlist
...
`gh ruleset check`，本 PR 不执行 apply。
