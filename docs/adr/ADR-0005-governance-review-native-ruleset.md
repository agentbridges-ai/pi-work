...
- 其他 PR（包括 Dependabot 等自动化作者）：`Misakago` 对当前 head 的有效审批计一票且每个 PR 只计一次；低风险 Dependabot 仍需 Leader 不是 head author/committer；
...
current-head 和 last-pusher 排除约束，从而允许 Leader self-or-exempt。每个 required status 绑定 GitHub Actions
