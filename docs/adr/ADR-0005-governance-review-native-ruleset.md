...
current-head 和实际最后 push 者排除约束，从而允许 Leader self-or-exempt。实际最后 push 者来自 trusted `synchronize` sender 或匹配当前 head/分支的仓库 `PushEvent` actor，PR `opened` opener 不被接受，无法读取时 fail-closed。每个 required status 绑定 GitHub Actions
