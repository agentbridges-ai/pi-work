...
并拒绝 Leader 同时作为实际最后 push 者的手工重放；该身份只由当前
`pull_request_target` 的 `synchronize` sender 或匹配当前 head/分支的 head 仓库 `PushEvent` actor 证明，PR `opened` opener 不被接受，缺失时 fail-closed；scope 不合规时回退
