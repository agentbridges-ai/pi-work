...
const officeRoot = resolve(
  process.env.PIWORK_ONLYOFFICE_BROWSER_DIR || join(root, "onlyoffice-browser"),
);
...
assert(hostArtifacts.length === 1, "release manifest must identify exactly one editor host bundle");
...
const optionalX2tRoot = process.env.PIWORK_ONLYOFFICE_X2T_DIR
  ...
  : resolve(root, "..", "onlyoffice-x2t-wasm");
if (existsSync(join(optionalX2tRoot, ".git"))) {
  assert(
