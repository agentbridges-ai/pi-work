...
describe("RC9: visibility-aware reconnection", () => {
  beforeEach(async () => {
    ...
    // Set up a session in the store (runtimeSessions is what visibility handler reads)
  ...
  it("disconnectAll cancels pending reconnect timers after the socket has closed", () => {
    connectSession("test-1");
    expect(wsInstances).toHaveLength(1);
    ...
    vi.advanceTimersByTime(5_000);
    expect(wsInstances).toHaveLength(1); // no reconnect from a stale timer
  });
  ...
  it("visibilitychange → visible reconnects current session even if runtimeSessions is stale", async () => {
    const { useStore } = await import("./store.js");

    // Simulate stale runtimeSessions list that doesn't include the current session.
    useStore.setState({
      currentSessionId: "test-1",
      runtimeSessions: [],
    });
