...
describe("logger", () => {
  ...
  const originalEnv = process.env.PIWORK_LOG_FORMAT;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env.PIWORK_LOG_FORMAT;
    } else {
      process.env.PIWORK_LOG_FORMAT = originalEnv;
    }
  });
  ...
  describe("human-readable format (default)", () => {
    ...
    it("redacts credentials and user content from diagnostic fields", () => {
      const spy = vi.spyOn(console, "log").mockImplementation(() => {});
      log.info("runtime", "launch", {
        sessionId: "s1",
        token: "capability-secret",
        nested: { prompt: "private user request", attempts: 2 },
      });

      const line = String(spy.mock.calls[0]?.[0]);
      expect(line).toContain("sessionId=s1");
      expect(line).toContain("[REDACTED]");
      expect(line).not.toContain("capability-secret");
      expect(line).not.toContain("private user request");
      spy.mockRestore();
    });
  ...
  describe("JSON format (PIWORK_LOG_FORMAT=json)", () => {
    beforeEach(async () => {
      process.env.PIWORK_LOG_FORMAT = "json";
      const mod = await import("./logger.js");
      log = mod.log;
    });
...
describe("LogFileWriter", () => {
  ...
  let tmpDir: string;
  const originalLogDir = process.env.PIWORK_LOG_DIR;
  const originalHome = process.env.PIWORK_HOME;

  beforeEach(async () => {
    vi.resetModules();
    // Create a unique temp directory for each test to avoid cross-contamination
    tmpDir = join(tmpdir(), `piwork-log-test-${randomBytes(4).toString("hex")}`);
    mkdirSync(tmpDir, { recursive: true });
    process.env.PIWORK_HOME = tmpDir;
    const mod = await import("./logger.js");
  ...
  afterEach(() => {
    ...
    if (originalLogDir === undefined) {
      delete process.env.PIWORK_LOG_DIR;
    } else {
      process.env.PIWORK_LOG_DIR = originalLogDir;
    }
    if (originalHome === undefined) {
      delete process.env.PIWORK_HOME;
    } else {
      process.env.PIWORK_HOME = originalHome;
    }
  });

  it("falls back to the default log directory when PIWORK_LOG_DIR is empty", () => {
    process.env.PIWORK_LOG_DIR = "";
    const writer = new LogFileWriter({ maxLines: 1_000_000 });
    try {
      expect(writer.getLogsDir()).toBe(join(tmpDir, "logs"));
      expect(writer.filePath).toContain(join(tmpDir, "logs"));
    } finally {
      writer.close();
    }
  });
  ...
  it("creates a log file in the specified directory", () => {
    ...
    try {
      ...
      if (process.platform !== "win32") {
        expect(statSync(writer.filePath).mode & 0o777).toBe(0o600);
  ...
  describe("rotation", () => {
    it("deletes oldest log files when total lines exceed maxLines", () => {
      ...
      const oldFile1 = join(tmpDir, "piwork_2020-01-01T00-00-00_1.log");
      const oldFile2 = join(tmpDir, "piwork_2020-06-01T00-00-00_2.log");
    ...
    it("does not delete the current log file during cleanup", () => {
      ...
      const oldFile = join(tmpDir, "piwork_2020-01-01T00-00-00_1.log");
    ...
    it("returns the number of files deleted during cleanup", () => {
      ...
      for (let i = 0; i < 3; i++) {
        const f = join(tmpDir, `piwork_2020-0${i + 1}-01T00-00-00_${i}.log`);
        writeFileSync(f, "a\nb\nc\nd\ne\n");
  ...
  describe("isEnabled", () => {
    const origLogFile = process.env.PIWORK_LOG_FILE;

    afterEach(() => {
      if (origLogFile === undefined) {
        delete process.env.PIWORK_LOG_FILE;
      } else {
        process.env.PIWORK_LOG_FILE = origLogFile;
      }
    });
    ...
    it("returns false when PIWORK_LOG_FILE=0", async () => {
      process.env.PIWORK_LOG_FILE = "0";
      vi.resetModules();
      const mod = await import("./logger.js");
      expect(mod.LogFileWriter.isEnabled()).toBe(false);
    });

    it("returns false when PIWORK_LOG_FILE=false", async () => {
      process.env.PIWORK_LOG_FILE = "false";
      vi.resetModules();
      const mod = await import("./logger.js");
      expect(mod.LogFileWriter.isEnabled()).toBe(false);
    });
...
describe("initLogFile / closeLogFile", () => {
  ...
  const origLogFile = process.env.PIWORK_LOG_FILE;
  const origLogFormat = process.env.PIWORK_LOG_FORMAT;

  beforeEach(async () => {
    vi.resetModules();
    tmpDir = join(tmpdir(), `piwork-log-init-${randomBytes(4).toString("hex")}`);
    mkdirSync(tmpDir, { recursive: true });
    delete process.env.PIWORK_LOG_FILE;
    delete process.env.PIWORK_LOG_FORMAT;
    const mod = await import("./logger.js");
  ...
  afterEach(() => {
    ...
    if (origLogFormat === undefined) {
      ...
      process.env.PIWORK_LOG_FORMAT = origLogFormat;
    }
  });
