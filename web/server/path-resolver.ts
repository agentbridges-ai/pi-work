 ...
 *
 * When Piwork runs as a macOS launchd or Linux systemd service, it inherits
...
export function captureUserShellPath(): string {
  try {
    const shell = environment.value(ENV.SHELL) || "/bin/bash";
    const captured = execSync(`${shell} -lic 'echo "___PATH_START___$PATH___PATH_END___"'`, {
      encoding: "utf-8",
      timeout: 10_000,
      env: { HOME: homedir(), USER: environment.value(ENV.USER), SHELL: shell },
    });
...
export function buildFallbackPath(): string {
  ...
  const nvmDir = environment.value(ENV.NVM_DIR) || join(home, ".nvm");
...
export function getEnrichedPath(): string {
  ...
  const currentPath = environment.value(ENV.PATH) || "";
...
export function resolveBinary(name: string): string | null {
  ...
  if (!sanitized || sanitized !== name) return null;

  // SRT is a pinned application dependency. Prefer that exact package over a
  // same-named executable earlier in the operator's shell PATH.
  if (sanitized === "srt") {
    if (process.platform === "win32") return null;
    const bundledSrt = join(
    ...
    return existsSync(bundledSrt) ? bundledSrt : null;
  }
  ...
  for (const cmd of commands) {
    try {
      const result = execSync(`${cmd} ${sanitized}`, {
        encoding: "utf-8",
        timeout: 5_000,
        env: environment.snapshot({ [ENV.PATH]: enrichedPath }),
      }).trim();
      if (!result) continue;
