// In-memory runtime metrics collector for the Piwork server.
...
export class MetricsCollector {
  ...
  private wsConnections = { piRpcOpened: 0, piRpcClosed: 0, browserOpened: 0, browserClosed: 0 };
  ...
  private wireEventBus(): void {
    this.unsubscribers.push(
      piworkBus.on("session:phase-changed", ({ sessionId, from, to }) => {
      ...
      piworkBus.on("session:exited", ({ sessionId, exitCode }) => {
      ...
      piworkBus.on("message:result", ({ sessionId }) => {
  ...
  recordPiRpcConnection(event: "open" | "close"): void {
    if (event === "open") this.wsConnections.piRpcOpened++;
    else this.wsConnections.piRpcClosed++;
  }

  recordWsConnection(_kind: "browser", event: "open" | "close"): void {
  ...
  reset(): void {
    ...
    this.wsConnections = { piRpcOpened: 0, piRpcClosed: 0, browserOpened: 0, browserClosed: 0 };
