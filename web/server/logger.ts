// Lightweight structured logger for the Piwork server.
...
//   By default, all log output is also written to ~/.piwork/logs/ with
//   automatic rotation (oldest files deleted when total lines exceed 2M).
//   Disable with PIWORK_LOG_FILE=0, override dir with PIWORK_LOG_DIR,
//   and configure rotation with PIWORK_LOG_MAX_LINES.
//
// Usage:
...
const STRUCTURED = environment.value(ENV.PIWORK_LOG_FORMAT) === "json";
const REDACTED = "[REDACTED]";
const SENSITIVE_LOG_KEY =
  /^(?:authorization|cookie|set-cookie|password|passphrase|token|access_?token|credential|secret|prompt|content|body|raw|raw_?preview|input|output|file_?content)$/i;

/** Keep diagnostic correlation fields while removing user or credential data. */
export function redactDiagnosticData(
  ...
  const visit = (value: unknown, key: string, depth: number): unknown => {
    ...
    if (Array.isArray(value)) return value.slice(0, 20).map((entry) => visit(entry, "", depth + 1));
    ...
    if (typeof value === "string" && value.length > 2_048) return `${value.slice(0, 2_048)}…`;
    return value;
  };
...
function formatEntry(
  ...
  const safeData = redactDiagnosticData(data);
  const correlations = requestContext
    ? {
        ...(!safeData?.requestId ? { requestId: requestContext.requestId } : {}),
        ...(!safeData?.userHash && requestContext.userHash
          ? { userHash: requestContext.userHash }
          : {}),
        ...(!safeData?.sessionId && requestContext.sessionId
          ? { sessionId: requestContext.sessionId }
          : {}),
        ...(!safeData?.contextEpoch && requestContext.contextEpoch !== undefined
          ? { contextEpoch: requestContext.contextEpoch }
          : {}),
      }
    : {};
  const correlatedData =
    Object.keys(correlations).length > 0 ? { ...safeData, ...correlations } : safeData;
  if (STRUCTURED) {
  ...
  let line = `[${module}] ${msg}`;
  if (correlatedData) {
    const pairs = Object.entries(correlatedData)
...
/**
 * Writes log lines to a file under ~/.piwork/logs/ with automatic rotation.
...
export class LogFileWriter {
  ...
  constructor(options?: { logsDir?: string; maxLines?: number }) {
    ...
    this.maxLines =
      options?.maxLines ?? environment.number(ENV.PIWORK_LOG_MAX_LINES, DEFAULT_LOG_MAX_LINES);
    ...
    this.filePath = join(this.logsDir, `piwork_${ts}_${pid}.log`);
  ...
  private static resolveDir(): string {
    return environment.optionalString(ENV.PIWORK_LOG_DIR, false) ?? join(PIWORK_HOME, "logs");
  }

  /** Whether log file writing is enabled. Disable with PIWORK_LOG_FILE=0|false. */
  static isEnabled(): boolean {
    const env = environment.value(ENV.PIWORK_LOG_FILE);
  ...
  cleanup(): number {
    try {
      ...
      if (deleted > 0) {
        ...
        console.log(
          `[logger] Cleanup: deleted ${deleted} old log file(s), ${totalLines} lines remaining`,
        );
      }
      return deleted;
    } catch {
 ...
 */
export function initLogFile(options?: {
  logsDir?: string;
  maxLines?: number;
}): LogFileWriter | null {
