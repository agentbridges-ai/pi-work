...
const mockReadFile = vi.hoisted(() => vi.fn(async (_path?: string, _encoding?: string) => ""));
const mockWriteFile = vi.hoisted(() =>
  vi.fn(async (_path: string, _content: string, _encoding?: string) => {}),
);
const mockRm = vi.hoisted(() => vi.fn(async () => {}));
const mockMkdir = vi.hoisted(() => vi.fn(async (_path?: string, _options?: unknown) => {}));
const mockLstat = vi.hoisted(() =>
  vi.fn(async () => ({
    dev: 1,
    ino: 1,
    isDirectory: () => true,
...
const mockRealpath = vi.hoisted(() => vi.fn(async (path: string) => path));
...
vi.mock("../path-policy.js", () => {
  class MockPathPolicyError extends Error {}
  return {
    PathPolicyError: MockPathPolicyError,
    isPathInside: () => true,
    ensureScopedDirectoryNoSymlink: async (path: string) => {
      await mockMkdir(path, { recursive: true, mode: 0o700 });
      return path;
    },
    readScopedFileNoFollow: async (path: string) => {
      if (!mockExistsSync(path)) {
        throw Object.assign(new Error("missing"), { code: "ENOENT" });
      }
      return Buffer.from(await mockReadFile(path, "utf-8"));
    },
    writeScopedFileNoFollow: async (
      path: string,
      content: string,
      _roots: readonly string[],
      options: { exclusive?: boolean } = {},
    ) => {
      if (options.exclusive && mockExistsSync(path)) {
...
beforeEach(() => {
  ...
  app = new Hono();
  registerSkillRoutes(app, { skillsDir: SKILLS_DIR });
});
...
describe("GET /skills", () => {
  ...
  it("parses front matter and returns skill metadata for valid skills", async () => {
    ...
    mockExistsSync.mockReturnValue(true);
    mockReaddir.mockResolvedValue([makeDirent("my-skill"), makeDirent("another-skill")] as any);
    mockReadFile
...
describe("POST /skills", () => {
  it("returns 507 before creating files when the user quota cache is unavailable", async () => {
    const quota = new UserDiskQuota({ maxBytes: 100, reservedHeadroomBytes: 10 });
    const quotaApp = new Hono();
    registerSkillRoutes(quotaApp, { skillsDir: SKILLS_DIR, diskQuota: quota });

    const res = await quotaApp.request("/skills", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: "Blocked Skill", content: "content" }),
    });
  ...
  it("creates a new skill with name, description, and content", async () => {
    ...
    expect(mockMkdir).toHaveBeenCalledWith(SKILLS_DIR, { recursive: true, mode: 0o700 });
    expect(mockMkdir).toHaveBeenCalledWith(`${SKILLS_DIR}/my-new-skill`, {
      recursive: true,
      mode: 0o700,
    });
...
describe("PUT /skills/:slug", () => {
  ...
  it("allows updating with empty string content", async () => {
    ...
    expect(mockWriteFile).toHaveBeenCalledWith(`${SKILLS_DIR}/my-skill/SKILL.md`, "", "utf-8");
  });
...
describe("DELETE /skills/:slug", () => {
  it("deletes the skill directory when it exists", async () => {
    ...
    expect(mockRm).toHaveBeenCalledWith(`${SKILLS_DIR}/doomed-skill`, {
      recursive: true,
      force: true,
    });
  });
