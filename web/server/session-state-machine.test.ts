...
describe("SessionStateMachine", () => {
  ...
  describe("valid transitions", () => {
    ...
    function expectValidTransition(from: SessionPhase, to: SessionPhase): void {
    ...
    it("starting -> initializing", () => expectValidTransition("starting", "initializing"));
    it("starting -> streaming", () => expectValidTransition("starting", "streaming"));
    it("starting -> reconnecting", () => expectValidTransition("starting", "reconnecting"));
    it("starting -> terminated", () => expectValidTransition("starting", "terminated"));

    // initializing -> ready, streaming, reconnecting, terminated
    it("initializing -> ready", () => expectValidTransition("initializing", "ready"));
    it("initializing -> streaming", () => expectValidTransition("initializing", "streaming"));
    it("initializing -> reconnecting", () => expectValidTransition("initializing", "reconnecting"));
    it("initializing -> terminated", () => expectValidTransition("initializing", "terminated"));

    // ready -> streaming, compacting, reconnecting, terminated
    it("ready -> streaming", () => expectValidTransition("ready", "streaming"));
    it("ready -> compacting", () => expectValidTransition("ready", "compacting"));
    it("ready -> reconnecting", () => expectValidTransition("ready", "reconnecting"));
    it("ready -> terminated", () => expectValidTransition("ready", "terminated"));

    // streaming -> ready, initializing, awaiting_permission, compacting, reconnecting, terminated
    it("streaming -> ready", () => expectValidTransition("streaming", "ready"));
    it("streaming -> initializing", () => expectValidTransition("streaming", "initializing"));
    it("streaming -> awaiting_permission", () =>
      expectValidTransition("streaming", "awaiting_permission"));
    it("streaming -> compacting", () => expectValidTransition("streaming", "compacting"));
    it("streaming -> reconnecting", () => expectValidTransition("streaming", "reconnecting"));
    it("streaming -> terminated", () => expectValidTransition("streaming", "terminated"));
    ...
    it("awaiting_permission -> ready", () => expectValidTransition("awaiting_permission", "ready"));
    ...
    it("compacting -> ready", () => expectValidTransition("compacting", "ready"));
    it("compacting -> streaming", () => expectValidTransition("compacting", "streaming"));
    it("compacting -> reconnecting", () => expectValidTransition("compacting", "reconnecting"));
    it("compacting -> terminated", () => expectValidTransition("compacting", "terminated"));

    // reconnecting -> initializing, starting, ready, streaming, terminated
    it("reconnecting -> initializing", () => expectValidTransition("reconnecting", "initializing"));
    it("reconnecting -> starting", () => expectValidTransition("reconnecting", "starting"));
    it("reconnecting -> ready", () => expectValidTransition("reconnecting", "ready"));
    it("reconnecting -> streaming", () => expectValidTransition("reconnecting", "streaming"));
    it("reconnecting -> terminated", () => expectValidTransition("reconnecting", "terminated"));

    // terminated -> starting
    it("terminated -> starting", () => expectValidTransition("terminated", "starting"));
  });
  ...
  describe("blocked transitions", () => {
    ...
    function expectBlockedTransition(from: SessionPhase, to: SessionPhase): void {
    ...
    it("starting -> ready is blocked", () => expectBlockedTransition("starting", "ready"));
    it("terminated -> ready is blocked", () => expectBlockedTransition("terminated", "ready"));
    ...
    // transitions (needed for Pi RPC reconnect recovery).
    it("ready -> initializing is blocked", () => expectBlockedTransition("ready", "initializing"));
  ...
  describe("guard methods", () => {
    ...
    describe("canSendToCLI()", () => {
      it("returns false for terminated, reconnecting, starting", () => {
        ...
        const unreachable: SessionPhase[] = ["terminated", "reconnecting", "starting"];
  ...
  describe("full lifecycle scenario", () => {
    it("walks through a complete session lifecycle", () => {
      ...
      // CLI connects
      expect(sm.transition("initializing", "pi-rpc-connected")).toBe(true);
      expect(sm.phase).toBe("initializing");
      ...
      expect(sm.transition("awaiting_permission", "tool-control-request")).toBe(true);
      expect(sm.phase).toBe("awaiting_permission");
      ...
      // CLI reconnects
      expect(sm.transition("initializing", "pi-rpc-reconnected")).toBe(true);
      expect(sm.phase).toBe("initializing");
      expect(sm.canSendToCLI()).toBe(true);
    ...
    it("handles early user message: starting -> streaming -> initializing -> ready", () => {
      ...
      expect(earlyMsg.transition("initializing", "pi_rpc_open")).toBe(true);
