// Zero-dependency, strongly-typed internal event bus for the Piwork server.

import type { PiworkEventMap } from "./event-bus-types.js";
 ...
 */
export class EventBus<TMap extends Record<string, any> = PiworkEventMap> {
        ...
        try {
          ...
          if (result && typeof (result as Promise<void>).catch === "function") {
            (result as Promise<void>).catch((err) => {
              console.error(`[event-bus] Async handler error for "${String(event)}":`, err);
            });
          }
        } catch (err) {
    ...
    if (onces && onces.size > 0) {
      ...
      for (const handler of snapshot) {
        try {
          ...
          if (result && typeof (result as Promise<void>).catch === "function") {
            (result as Promise<void>).catch((err) => {
              console.error(`[event-bus] Async once-handler error for "${String(event)}":`, err);
            });
          }
        } catch (err) {
          console.error(`[event-bus] Once-handler error for "${String(event)}":`, err);
        }
      }
    }
  }
...
/** Singleton bus instance for the Piwork server. */
export const piworkBus = new EventBus<PiworkEventMap>();
