...
const MOCK_SERVICE_PATH =
  "/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/mock/.bun/bin:/mock/.local/bin";
...
function plistPath(): string {
  return join(tempDir, "Library", "LaunchAgents", "app.piwork.workbench.plist");
}

function oldPlistPath(): string {
  return join(tempDir, "Library", "LaunchAgents", "co.thevibecompany.piwork.plist");
}

function unitPath(): string {
  return join(tempDir, ".config", "systemd", "user", "piwork.service");
}

function logDir(): string {
  return join(tempDir, ".piwork", "logs");
}
...
describe("generatePlist", () => {
  it("generates valid XML with the correct label", () => {
    const plist = service.generatePlist({ binPath: "/usr/local/bin/piwork" });
    expect(plist).toContain('<?xml version="1.0"');
    expect(plist).toContain("<string>app.piwork.workbench</string>");
  });
  ...
  it("uses the provided binary path in ProgramArguments", () => {
    const plist = service.generatePlist({ binPath: "/opt/homebrew/bin/piwork" });
    expect(plist).toContain("<string>/opt/homebrew/bin/piwork</string>");
  ...
  it("uses a custom port when specified", () => {
    const plist = service.generatePlist({ binPath: "/usr/local/bin/piwork", port: 8080 });
  ...
  it("uses custom path option when provided", () => {
    ...
    const plist = service.generatePlist({ binPath: "/usr/local/bin/piwork", path: customPath });
...
describe("generateSystemdUnit", () => {
  it("generates a valid systemd unit with correct sections", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/piwork" });
  ...
  it("includes the description", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/piwork" });
    expect(unit).toContain("Description=Piwork");
  });

  it("uses the provided binary path in ExecStart", () => {
    const unit = service.generateSystemdUnit({ binPath: "/home/user/.bun/bin/piwork" });
    expect(unit).toContain("ExecStart=/home/user/.bun/bin/piwork start --foreground");
  });
  ...
  it("uses a custom port when specified", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/piwork", port: 8080 });
  ...
  it("uses custom path option when provided", () => {
    const customPath = "/custom/bin:/other/bin";
    const unit = service.generateSystemdUnit({
      binPath: "/usr/local/bin/piwork",
      path: customPath,
    });
...
describe("install", () => {
  it("creates log directory and writes plist file", async () => {
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which")) return "/usr/local/bin/piwork\n";
    ...
    expect(content).toContain("app.piwork.workbench");
  });
  ...
  it("migrates old launchd label before installing", async () => {
    ...
    const plist = service
      .generatePlist({ binPath: "/usr/local/bin/piwork" })
      .replaceAll("app.piwork.workbench", "co.thevibecompany.piwork");
...
describe("install (linux)", () => {
  ...
  it("creates log directory and writes systemd unit file", async () => {
    ...
    expect(content).toContain("ExecStart=/usr/local/bin/piwork start --foreground");
  });
...
describe("start (linux)", () => {
  ...
  it("calls systemctl start when installed", async () => {
    ...
    const startCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("start piwork.service"),
    );
    expect(startCall).toBeDefined();
  });
...
describe("stop (linux)", () => {
  ...
  it("calls systemctl stop when installed", async () => {
    ...
    const stopCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("stop piwork.service"),
    );
    expect(stopCall).toBeDefined();
  });
...
describe("restart (linux)", () => {
  ...
  it("calls systemctl restart when installed", async () => {
    ...
    const restartCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("restart piwork.service"),
    );
    expect(restartCall).toBeDefined();
  });
...
`,
    ...
    mockExecSync.mockImplementation((cmd: string) => {
      if (typeof cmd === "string" && cmd.includes("launchctl list")) {
        return `{\n\t"PID" = 12345;\n\t"Label" = "co.thevibecompany.piwork";\n}`;
      }
      return "";
    });
...
describe("status (linux)", () => {
  ...
  it("returns installed: true, running: true with PID", async () => {
    ...
    mockExecSync.mockReset();
    mockExecSync.mockImplementation((cmd: string) => {
      if (typeof cmd === "string" && cmd.includes("show piwork.service")) {
...
describe("refreshServiceDefinition (macOS)", () => {
  it("rewrites plist with current binary path", async () => {
    ...
    expect(originalContent).toContain("/usr/local/bin/piwork");
    ...
    mockExecSync.mockReset();
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which")) return "/new/path/piwork\n";
      return "";
    });
    ...
    expect(updatedContent).toContain("/new/path/piwork");
  });
