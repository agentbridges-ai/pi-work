...
describe("counters", () => {
  it("tracks sessions created by backend type", () => {
    collector.recordSessionCreated("pi");
    collector.recordSessionCreated("pi");
    collector.recordSessionCreated("pi");

    const snap = collector.getSnapshot();
    expect(snap.counters.sessionsCreated).toEqual({ pi: 3 });
  });

  it("tracks sessions terminated by exit code via event bus", () => {
    piworkBus.emit("session:exited", { sessionId: "s1", exitCode: 0 });
    piworkBus.emit("session:exited", { sessionId: "s2", exitCode: 1 });
    piworkBus.emit("session:exited", { sessionId: "s3", exitCode: null });
    piworkBus.emit("session:exited", { sessionId: "s4", exitCode: 0 });
  ...
  it("tracks native Pi RPC and browser WebSocket connections", () => {
    collector.recordPiRpcConnection("open");
    collector.recordPiRpcConnection("open");
    collector.recordPiRpcConnection("close");
