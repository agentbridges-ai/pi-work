...
describe("buildFallbackPath", () => {
  ...
  it("includes ~/.local/bin for user-local executables", () => {
    mockExistsSync.mockImplementation(
      (p: string) => p === "/home/testuser/.local/bin" || p === "/usr/bin",
    );
  ...
  it("includes ~/.bun/bin", () => {
    mockExistsSync.mockImplementation(
      (p: string) => p === "/home/testuser/.bun/bin" || p === "/usr/bin",
    );
  ...
  it("includes ~/.cargo/bin for Rust tools", () => {
    mockExistsSync.mockImplementation(
      (p: string) => p === "/home/testuser/.cargo/bin" || p === "/usr/bin",
    );
...
describe("resolveBinary", () => {
  ...
  it("returns absolute path when binary is found via which", () => {
    ...
    mockExecSync.mockImplementation((cmd: string) => {
      ...
      if (typeof cmd === "string" && cmd.startsWith("which mytool")) {
        return "/home/testuser/.local/bin/mytool\n";
      }
      throw new Error("not found");
    });

    expect(resolveBinary("mytool")).toBe("/home/testuser/.local/bin/mytool");
  });
  ...
  it("prefers the pinned local SRT package over the shell PATH", () => {
    if (process.platform === "win32") return;
    mockExistsSync.mockImplementation((candidate: string) =>
      candidate.endsWith("/node_modules/.bin/srt"),
    );

    const result = resolveBinary("srt");

    expect(result).toMatch(/\/node_modules\/\.bin\/srt$/);
    expect(mockExecSync).not.toHaveBeenCalledWith(
      expect.stringMatching(/^which srt/),
      expect.anything(),
    );
  });

  it("never falls back to a same-named SRT wrapper from PATH", () => {
    if (process.platform === "win32") return;
    mockExistsSync.mockReturnValue(false);
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which srt")) return "/tmp/untrusted/srt\n";
      return "___PATH_START___/usr/bin___PATH_END___\n";
    });

    expect(resolveBinary("srt")).toBeNull();
    expect(mockExecSync).not.toHaveBeenCalledWith(
      expect.stringMatching(/^which srt/),
      expect.anything(),
    );
  ...
  it("rejects binary names that require shell sanitization", () => {
    expect(resolveBinary("git;touch-pwned")).toBeNull();
  });
  ...
  it("returns the path directly when given an absolute path that exists", () => {
    mockExistsSync.mockReturnValue(true);
    expect(resolveBinary("/opt/bin/mytool")).toBe("/opt/bin/mytool");
  });

  it("returns null when given an absolute path that does not exist", () => {
    mockExistsSync.mockReturnValue(false);
    expect(resolveBinary("/nonexistent/mytool")).toBeNull();
  });
  ...
  describe("Windows support", () => {
    ...
    it("returns null for a non-existent Windows absolute path", () => {
      mockExistsSync.mockReturnValue(false);
      expect(resolveBinary("D:\\nonexistent\\mytool.cmd")).toBeNull();
    });
    ...
    it("prefers 'where' over 'which' on Windows when both succeed", () => {
      ...
      mockExecSync.mockImplementation((cmd: string) => {
        ...
        if (typeof cmd === "string" && cmd.startsWith("which")) {
          return "/c/Users/me/AppData/Roaming/npm/mytool";
        }
        throw new Error("not found");
      });

      // Should return the 'where' result (native Win32 path), not the 'which' POSIX path
      expect(resolveBinary("mytool")).toBe("C:\\Users\\me\\AppData\\Roaming\\npm\\mytool.cmd");
    });
    ...
    it("falls back to 'which' when 'where' fails on Windows", () => {
      ...
      expect(resolveBinary("mytool")).toBe("/c/Users/me/AppData/Roaming/npm/mytool");
    });
