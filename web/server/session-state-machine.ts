// Formal session state machine for the Piwork server.
 ...
 *
 * - starting:            Per-session Pi rpc-entry process is being spawned
 * - initializing:        Pi RPC connected, awaiting readiness probes
 * - ready:               Idle, awaiting user input
 * - streaming:           Pi generating output
 * - awaiting_permission: Tool call pending user approval
 * - compacting:          Context window compaction in progress
 * - reconnecting:        Pi RPC transport dropped, within grace period
 ...
 */
export const VALID_TRANSITIONS: ReadonlyMap<SessionPhase, ReadonlySet<SessionPhase>> = new Map([
  ["starting", new Set<SessionPhase>(["initializing", "streaming", "reconnecting", "terminated"])],
  ["initializing", new Set<SessionPhase>(["ready", "streaming", "reconnecting", "terminated"])],
  ["ready", new Set<SessionPhase>(["streaming", "compacting", "reconnecting", "terminated"])],
  [
    "streaming",
  ...
  ["compacting", new Set<SessionPhase>(["ready", "streaming", "reconnecting", "terminated"])],
  [
    "reconnecting",
...
export class SessionStateMachine {
  ...
  /** True when the Pi RPC transport is expected to be reachable. */
  canSendToCLI(): boolean {
    return (
      this._phase !== "terminated" && this._phase !== "reconnecting" && this._phase !== "starting"
    );
  }
