...
const LOG_DIR = join(PIWORK_HOME, "logs");
const STDOUT_LOG = join(LOG_DIR, "piwork.log");
const STDERR_LOG = join(LOG_DIR, "piwork.error.log");

// ─── macOS (launchd) Constants ──────────────────────────────────────────────────

const LABEL = "app.piwork.workbench";
const OLD_LABEL = "co.thevibecompany.piwork";
...
const UNIT_NAME = "piwork.service";
...
export function generateSystemdUnit(opts: UnitOptions): string {
  ...
  return `[Unit]
Description=Piwork - Native Pi agent workbench
...
function resolveBinPath(): string {
  try {
    const binPath = execSync("which piwork", { encoding: "utf-8" }).trim();
    if (binPath) return binPath;
  } catch {
    // not found globally
  }

  console.error("piwork CLI was not found on PATH for local service mode.");
  console.error("");
  console.error("Run the workbench from the repository, or add your local CLI shim to PATH.");
  console.error("");
  console.error("Then retry:");
  console.error("");
  console.error("  piwork install");
  process.exit(1);
}
...
function migrateLegacyInstallIfNeeded(): void {
  ...
  console.log("Found legacy The Vibe Piwork service. Migrating...");
...
function systemctlUser(cmd: string): string {
  ...
  return execSync(`systemctl --user ${cmd}`, {
    ...
    env: environment.snapshot({
      [ENV.XDG_RUNTIME_DIR]: environment.value(ENV.XDG_RUNTIME_DIR) || `/run/user/${uid}`,
    }),
  });
}
...
async function installDarwin(opts?: { port?: number }): Promise<void> {
  ...
  if (existsSync(PLIST_PATH)) {
    console.error("Piwork is already installed as a service.");
    console.error("Run 'piwork uninstall' first to reinstall.");
    process.exit(1);
  }
  ...
  console.log("Piwork has been installed as a background service.");
  console.log("");
  ...
  console.log("Use 'piwork status' to check if it's running.");
}
...
async function uninstallDarwin(): Promise<void> {
  ...
  if (!installedService) {
    console.log("Piwork is not installed as a service.");
    return;
  }
  ...
  console.log("Piwork service has been removed.");
...
async function startDarwin(): Promise<void> {
  ...
  if (!installedService) {
    console.log("Piwork is not installed as a service.");
    console.log("Run 'piwork install' first.");
    return;
  }
  ...
  const domainTarget =
    uid !== undefined ? `gui/${uid}/${installedService.label}` : installedService.label;

  try {
    execSync(`launchctl kickstart -k "${domainTarget}"`, { stdio: "pipe" });
  } catch {
    try {
      execSync(`launchctl bootstrap "${domain}" "${installedService.plistPath}"`, {
        stdio: "pipe",
      });
    } catch {
      try {
  ...
  console.log("Piwork service has been started.");
}
...
async function stopDarwin(): Promise<void> {
  ...
  console.log("Piwork service has been stopped.");
  console.log("Run 'piwork restart' to start it again.");
}
...
async function restartDarwin(): Promise<void> {
  ...
  console.log("Piwork service has been restarted.");
}
