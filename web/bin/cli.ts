...
environment.set(ENV.__PIWORK_PACKAGE_ROOT, resolve(__dirname, ".."));

const command = process.argv[2];

// Management subcommands that delegate to ctl.ts
const CTL_COMMANDS = new Set(["sessions", "cron", "skills", "assistant", "ctl-help"]);

function printUsage(): void {
  console.log(`
Usage: piwork [command]
...
Server commands:
  ...
  status      Show service status (or use 'piwork status' when server is running)
...
Management commands (requires running server):
  ...
  skills      Manage governed Pi skills (list, get, create, update, delete)
  assistant   Manage Piwork Assistant (status, launch, stop, config)
...
switch (command) {
  ...
  case "serve": {
    environment.setDefault(ENV.NODE_ENV, "production");
  ...
  case "status": {
    ...
    try {
      ...
      if (!result.installed) {
        ...
        console.log("Run: piwork install");
      } else if (result.running) {
        console.log(`Piwork is running (PID: ${result.pid})`);
        console.log(`  URL: http://localhost:${result.port}`);
      } else {
        console.log("Piwork is installed but not running.");
        console.log("Check logs at ~/.piwork/logs/");
      }
    }
    break;
  }
  ...
  case "logs": {
    ...
    const logFile = join(homedir(), ".piwork/logs/piwork.log");
    const errFile = join(homedir(), ".piwork/logs/piwork.error.log");
    const { existsSync } = await import("node:fs");
    if (!existsSync(logFile) && !existsSync(errFile)) {
      console.error("No log files found at ~/.piwork/logs/");
      console.error("The service may not have been started yet.");
      process.exit(1);
    }
    console.log("Tailing logs from ~/.piwork/logs/");
