...
function isSrtNetworkSocketPath(value) {
  return /^\/tmp\/piwork-pi-[^/]+\/proxy\/claude-(?:http|socks)-[^/]+\.sock$/u.test(value);
}

function srtProxyDirectoryForSocket(value) {
  const match = value.match(
    /^(\/tmp\/piwork-pi-[^/]+\/proxy)\/claude-(?:http|socks)-[^/]+\.sock$/u,
  );
  return match?.[1];
}
  ...
  try {
    ...
    for (const component of parent.split("/").filter(Boolean)) {
      ...
      const nextFd = openSync(
        ...
        directoryFlags | constants.O_CLOEXEC,
      );
      closeSync(parentFd);
      parentFd = nextFd;
...
function closeProxyDirectoryFds() {
...
for (let index = 0; index < input.length; index += 1) {
  ...
  if (
    ...
    deferredNetworkBinds.push({
      source: input[index + 1],
      target: input[index + 2],
      proxyDirectory: srtProxyDirectoryForSocket(input[index + 1]),
    });
    index += 2;
    continue;
  }
...
// Keep its host-side proxy sockets visible by re-binding their dedicated
// manager directory from an already-open directory FD after those mounts have
// been constructed. The paths are generated by SRT itself; no user-controlled
// bind is reordered and no socket source path is mounted individually.
...
try {
  const proxyDirectories = [
    ...new Set(deferredNetworkBinds.map(({ proxyDirectory }) => proxyDirectory).filter(Boolean)),
  ];
  for (const proxyDirectory of proxyDirectories) {
    const fd = openProxyDirectoryFd(proxyDirectory);
    if (fd === undefined) {
      throw new Error(`piwork-bwrap: generated proxy directory is unavailable: ${proxyDirectory}`);
    }
    const childFd = 3 + proxyDirectoryFds.length;
    proxyDirectoryFds.push(fd);
    networkBindArgs.push("--bind-fd", String(childFd), proxyDirectory);
  }
} catch (error) {
  closeProxyDirectoryFds();
  throw error;
}
